(module
  ...
  (func $set_pixel (param $x i32) (param $y i32) (param $c i32)
    ;; check bounds
  ...
  (func $draw_obj (param $x i32) (param $y i32) (param $c i32)
    (local $max_x i32)
    (local $max_y i32)
      ...
      (block $outer_break
        ...
        (loop $inner_loop
          (block $inner_break
            ...
            global.get $obj_size
            i32.ge_u
            if ;; x dist > object size, then no collision
              local.get $j
              i32.const 1
              i32.add
              local.set $j
            ...
            global.get $obj_size
            i32.ge_u
            if ;; y dist > object size, then no collision
              local.get $j
              i32.const 1
              i32.add
              local.set $j
            ...
            i32.const 1
            local.set $i_hit
            ;; exit loop if condition, no need to check rest of objects
          )
        )
